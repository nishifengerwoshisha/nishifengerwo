...
export function initEditor(monacoEditor: typeof Monaco): void {
  // 注册 mermaid语言到monaco
  monacoEditor.languages.register({ id: 'mermaid' });
  /**
   * 定义各类 Mermaid 图表所使用的关键词列表。
   * 包括：
   * - typeKeywords: 表示图表类型的关键词（例如 graph、sequenceDiagram）
   * - blockKeywords: 控制结构或区块开始的关键词（例如 subgraph、class）
   * - keywords: 其他功能性关键词（例如 title、direction）
   */
  ...
  /**
   * 处理 %%{...}%} 这种形式的配置指令，将其嵌入 JavaScript 内容进行解析。
   */
  ...
  /**
   * 设置 Mermaid 语言的词法分析规则（Monarch Tokens Provider），包括各种图表类型的语法高亮逻辑。
   * 使用 Monarch 语法描述格式来匹配不同上下文中的关键字、字符串、注释等元素并赋予对应 Token 类型。
   */
  ...
  /**
   * 设置 Mermaid 语言的基本编辑行为配置，比如注释符号、括号配对等。
   */
