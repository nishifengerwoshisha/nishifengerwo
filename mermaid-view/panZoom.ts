import Hammer, { type Manager as HammerManager } from 'hammerjs';
import type { Point } from 'mermaid/dist/types.js';
import panzoom from 'svg-pan-zoom';
import type { State } from './mermaid-types';
import { updateMermaidStateHandler } from '@/tools/mermaid-view/mermaid-state';
...
export class PanZoomState {
  ...
  public onPanZoomChange?: (pan: Point, zoom: number) => void = (pan, zoom) => {
    // 将平移缩放状态保存到state
    updateMermaidStateHandler({
      pan,
      zoom,
    });
  };
  ...
  public updateElement(diagramView: SVGElement, {
    pan,
    zoom,
  }: Pick<State, 'pan' | 'zoom'>) {
    this.pzoom?.destroy();
